  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <div className="mx-auto max-w-6xl px-4 py-7 sm:px-8 sm:py-10">
        ...
        <section className="mb-7 grid gap-6 sm:grid-cols-[1fr_auto] sm:items-end">
          <div>
            <p className="mb-1 text-center text-sm font-medium uppercase tracking-[0.14em] text-slate-500 sm:text-left">Inventario actual</p>
            <h1 className="text-center text-3xl font-semibold tracking-tight sm:text-left sm:text-4xl">EXPLOSIVOS Y ACCESORIOS</h1>
            <div className="mt-5 grid gap-3 [&>button]:w-full [&>button]:justify-center sm:flex sm:flex-wrap sm:[&>button]:w-auto"><RecordDialog initialType="INDUGEL" onSaved={async result=>{await loadRecords();if(result?.loteIngreso)setVerificationSignal(value=>value+1);}} triggerLabel="REGISTRAR INGRESOS" triggerClassName="h-12 bg-[#0d2c3e] px-5 font-semibold text-white hover:bg-[#16445d]"/><MovementDialog records={records} onSaved={loadRecords}/><VerificationDialog records={records} onSaved={loadRecords} autoOpenSignal={verificationSignal}/>{currentUser?.rol === "ADMINISTRADOR" && <AccessDialog/>}</div>
          </div>
          ...
          {modules.map(({ name, tone }) => {
            ...
            <div key={name} className={`relative rounded-2xl border bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${selected === name ? "border-[#f5b51b] ring-2 ring-[#f5b51b]/20" : "border-slate-200"}`}>
              <button type="button" onClick={() => {setSelected(name);if(name!=="ANFO")setMaterialViews(current=>({...current,[name]:(view+1)%((name==="DETONADORES"||name==="MECHA DE SEGURIDAD")?3:2)}));}} className="group block w-full text-left" aria-label={`${name}. ${measureHint(name,view)}`}>
                ...
                <span className="block pr-7 text-base font-bold leading-tight">{name}</span>
