      ...
      <div className="mx-auto max-w-6xl px-4 py-7 sm:px-8 sm:py-10">
        ...
        <section className="mt-7">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            </div> : <div className="divide-y divide-slate-100">{selectedRecords.map((record) => <article key={String(record.id)} className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{String(record.serial || record.cajaNumero || `Registro ${record.id}`)}</p>{record.verificado===false&&<span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-800">PENDIENTE DE VERIFICAR</span>}</div><p className="text-sm text-slate-500">{selected==="MECHA DE SEGURIDAD"?`${String(record.cantidad||record.contenido||2)} bobinas`:String(record.loteProduccion || record.contenido || "Registro individual")}</p>{selected!=="SELLOS"&&<p className="mt-1 text-xs text-slate-500">Ingreso: {String(record.fechaIngreso||"")} · Fabricación: {String(record.fechaFabricacion||record.fechaProduccion||"")} · Vencimiento: {String(record.fechaVencimiento||"")}</p>}</div>{selected!=="SELLOS"&&<div className="flex items-center gap-3 sm:justify-end"><span className="flex items-center gap-2 text-sm text-slate-600"><MapPin className="h-4 w-4" />{String(record.ubicacion||"")}</span>{currentUser?.rol==="ADMINISTRADOR"&&<Button type="button" variant="outline" size="sm" onClick={()=>void removeRecord(record)} className="border-red-200 text-red-700 hover:bg-red-50 hover:text-red-800" title={`Eliminar ${String(record.serial||record.cajaNumero||record.id)}`}><Trash2 className="h-4 w-4"/><span className="hidden lg:inline">Eliminar</span></Button>}</div>}</article>)}</div>}
          </div>
