    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <header className="border-b border-slate-800 bg-[#0d2c3e] text-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4 sm:px-8">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2"><Button type="button" onClick={()=>void loadRecords()} disabled={syncState==="syncing"} className="h-12 min-w-32 flex-col gap-0 rounded-lg bg-white px-3 text-[#0d2c3e] hover:bg-slate-100"><span className="flex items-center gap-1.5 text-xs font-semibold">{syncState==="syncing"?<RefreshCw className="h-4 w-4 animate-spin"/>:syncState==="success"?<Check className="h-4 w-4 text-emerald-600"/>:syncState==="error"?<AlertTriangle className="h-4 w-4 text-red-600"/>:<RefreshCw className="h-4 w-4"/>}SINCRONIZAR{pendingSync>0&&<span className="rounded-full bg-amber-400 px-1.5 py-0.5 text-[10px]">{pendingSync}</span>}</span><span className="mt-0.5 text-[9px] font-medium text-slate-500">{lastSync?`Sinc. ${lastSync}`:"Sin sincronización"}</span></Button><InstallAppButton/>{currentUser?.rol === "ADMINISTRADOR" && <AccessDialog/>}<div className="max-w-40 text-center"><p className="truncate px-2 text-[11px] font-medium text-slate-200" title={currentUser?.nombre||currentUser?.usuario}>{currentUser?.nombre||currentUser?.usuario}</p><Button variant="ghost" className="h-8 text-white hover:bg-white/10 hover:text-white" onClick={() => { logout(); setAuthenticated(false); setCurrentUser(null); }}><LogOut className="h-4 w-4" /><span className="hidden sm:inline">Cerrar sesión</span></Button></div></div>
        </div>
      </header>
