...
type InstallWindow=Window&{__explosivosInstallPrompt?:InstallPromptEvent|null};
...
export function InstallAppButton(){
  ...
  useEffect(()=>{
    ...
    useCaptured();window.addEventListener("beforeinstallprompt",ready);window.addEventListener("explosivosinstallprompt",useCaptured);window.addEventListener("appinstalled",done);return()=>{window.removeEventListener("beforeinstallprompt",ready);window.removeEventListener("explosivosinstallprompt",useCaptured);window.removeEventListener("appinstalled",done);};
  },[]);
  if(installed)return null;
  async function install(){const available=prompt||(window as InstallWindow).__explosivosInstallPrompt;if(available){try{await available.prompt();const choice=await available.userChoice;if(choice.outcome==="accepted"){setPrompt(null);(window as InstallWindow).__explosivosInstallPrompt=null;}}catch{setHelp(true)}}else setHelp(true);}
